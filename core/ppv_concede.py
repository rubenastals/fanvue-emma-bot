...
def _defend_hits_needed() -> int:
    """How many cheaper-asks to DEFEND before conceding (default 2)."""
    return max(1, int(getattr(config, "PPV_PRICE_DEFEND_HITS", 2) or 2))
...
def pick_cheaper_offer(
    ...
    *,
    ...
    """Cheapest L1–L2 under the active lock price."""
    ...
    if not under:
        # Fall back to vault cheap band directly
        try:
            items = [
                dict(i)
                ...
                and 0 < float(i.get("price") or 0) < max(current_price, 0.01)
                and i.get("media_uuid") not in set(mem.get("sent_media_uuids") or [])
            ]
            under = items
        except Exception:
            under = []
...
def evaluate(
    *,
    ...
    Does NOT bump hits — caller uses bump_and_plan for that.
    """
    if not unpaid:
        return ConcedePlan(PHASE_NONE, 0, "no unpaid lock")
    if not fan_asks_cheaper(fan_message):
        return ConcedePlan(PHASE_NONE, int(mem.get("price_defend_hits") or 0), "not a cheaper ask")
    ...
    hits = int(mem.get("price_defend_hits") or 0) + 1
    msg_uuid = lock_message_uuid(ppv_status, mem)
    need = _defend_hits_needed()
